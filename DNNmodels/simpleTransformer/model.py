...
class Transformer(nn.Module):
    def __init__(self, input_dim, model_dim, num_heads, num_layers, num_classes, dropout = 0.1):
        super(Transformer, self).__init__() #initialize torch nn.Module
        self.embedding = nn.Embedding(input_dim, model_dim) # embedding

        encoder_layer = nn.TransformerEncoderLayer(
            d_model = model_dim,
            nhead = num_heads,
            dim_feedforward = 4 * model_dim, #from attention is all you need
            dropout = dropout,
            batch_first = True # (Batch, Seq_len, Dim) order
        )
