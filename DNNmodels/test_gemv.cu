...
__global__ void timedSpinKernel(int N, unsigned long long maxCycles, const float* A, const float* x) {
...
void launchPulse(int N, unsigned long long maxCycles, int blocks, int TPB, const float* devA, const float* devx) {
    timedSpinKernel<<<blocks, TPB>>>(N, maxCycles, devA, devx);
...
int main(int argc, char** argv) {
    ...
    cudaMalloc((void**)& devA, sizeof(float)*100000);
    cudaMalloc((void**)& devx, sizeof(float)*56);

    cudaMemcpy(devA, A, sizeof(float)*100000, cudaMemcpyDeviceToHost);
    cudaMemcpy(devx, x, sizeof(float)*56, cudaMemcpyDeviceToHost);
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        ...
        if (v == 0) {
            ...
            launchPulse(v * TPB, maxCycles/5, v, TPB, devA, devx);
            launchPulse(v * TPB, maxCycles/5, v, TPB, devA, devx);
            launchPulse(v * TPB, maxCycles/5, v, TPB, devA, devx);
            launchPulse(v * TPB, maxCycles/5, v, TPB, devA, devx);
            launchPulse(v * TPB, maxCycles/5, v, TPB, devA, devx);
