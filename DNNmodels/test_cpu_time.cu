...
int main(int argc, char** argv) {
    ...
    std::vector<std::tuple<std::string, int>> trace; // wall_time_str, symbol

    if (argc < 5) {
        std::cerr << "Usage: " << argv[0]
                  << " <pulse_ms> <delay_ms> <input_file.csv> <output.csv>\n"
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        ...
        // record v and cpu_time
        std::string wall_time_str = getCurrentTimeString();
        trace.emplace_back(wall_time_str, v);
    ...
    out << "wall_time_str,symbol\n";
    for (const auto& [wall, symbol] : trace) {
        out << wall << "," << symbol << "\n";
    }
