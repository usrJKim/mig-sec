...
__global__ void timedSpinKernel_GEMV(int N, const float* A, const float* x, int rows, int cols, unsigned long long maxCycles) {
    ...
    int j = 0;
    while (clock64() - start < maxCycles) {
        sum += A[(row * cols +j)%rows ] * x[j%rows];
        j++;
        // some non‑trivial work to hold the SM busy
    }
}
...
__global__ void timedSpinKernel_SFU(int N, unsigned long long maxCycles) {
    ...
    while (clock64() - start < maxCycles) {
        // some non‑trivial work to hold the SM busy
        val = sinf(val)+evalpf(val)*sqrtf(val);
    }
}
...
void launchPulse(int N, const float*A, const float* x, unsigned long long maxCycles, int blocks, int TPB) {
    // timedSpinKernel_sharedMem<<<blocks, TPB>>>(N, maxCycles);
    timedSpinKernel_GEMV<<<blocks, TPB>>>(N, A, x, N, N, maxCycles);
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
        ...
        if (v == 0) {
            ...
            launchPulse(v * TPB, d_A, d_x, maxCycles/5, v, TPB);
            launchPulse(v * TPB, d_A, d_x, maxCycles/5, v, TPB);
            launchPulse(v * TPB, d_A, d_x, maxCycles/5, v, TPB);
            launchPulse(v * TPB, d_A, d_x, maxCycles/5, v, TPB);
            launchPulse(v * TPB, d_A, d_x, maxCycles/5, v, TPB);
